import { QueryParameters } from "./types/client.js";

export const pathGetter =
  <T>(obj: T) =>
  (paths: (keyof any)[]) =>
    paths.reduce((acc, key) => acc && (acc as any)[key], obj);
  ...
  for (const [key, value] of Object.entries(query)) {
    ...
    if (Array.isArray(value)) {
      for (const v of value) {
        append(key, v instanceof Date ? v.toISOString() : v, true);
      }
    } else {
      append(key, `${value instanceof Date ? value.toISOString() : value}`);
    }
  }
